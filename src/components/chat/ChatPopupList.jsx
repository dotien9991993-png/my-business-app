    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b flex-shrink-0">
        ...
        <div className="flex items-center gap-1">
          {onOpenSettings && (
            <button onClick={onOpenSettings} className="w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-500" title="Cài đặt">
              ⚙️
            </button>
          )}
          {onOpenNewChat && (
            <button onClick={onOpenNewChat} className="w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-500" title="Tin nhắn mới">
              ✏️
            </button>
          )}
          {onOpenFullChat && (
            <button onClick={onOpenFullChat} className="w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-500" title="Mở chat đầy đủ">
          ...
          sortedRooms.map(room => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 transition-colors text-left hover:bg-gray-50 ${
                  unread > 0 ? 'bg-green-50/40' : ''
                }`}
              >
                {/* Avatar */}
                <div className="w-11 h-11 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-bold text-sm flex-shrink-0 overflow-hidden">
                  {info.isGroup ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[10px] flex-shrink-0 ml-1.5 ${unread > 0 ? 'text-green-600 font-medium' : 'text-gray-400'}`}>
                  ...
                  <div className="flex items-center justify-between mt-0.5">
                    ...
                    {unread > 0 && (
                      <span className="bg-red-500 text-white text-[9px] font-bold rounded-full min-w-[16px] h-[16px] flex items-center justify-center px-1 flex-shrink-0 ml-1.5">
                        {unread > 99 ? '99+' : unread}
                      </span>
                    )}
                  </div>
